...
/* ------------------------------------------------------------------ */
export async function getRecentParents() {
    try {
...
export async function addRecentParent(dir) {
...
function isPotentialTaskFile(name) {
    return name.endsWith(".yaml") || name.endsWith(".yml");
}
function isLegacyTaskFilename(name) {
    return (isPotentialTaskFile(name) &&
        (name.startsWith("zenarc-") || name.startsWith("tm-")));
}
function getZenarcDir(projectPath) {
    return join(projectPath, ".zenarc");
}
function getProjectOverviewPath(projectPath) {
    return join(getZenarcDir(projectPath), "overview.yml");
}
export class YamlTaskStore {
    getProjectTasksDir(projectPath) {
        return join(projectPath, ".zenarc", "tasks");
    }
    async ensureZenarcStructure(projectPath) {
    ...
    /*  Config / summary file                                             */
    /* ------------------------------------------------------------------ */
    async writeProjectConfig(projectPath, tasks) {
    ...
    async listProjectTasks(projectPath) {
        ...
        return Array.from(tasks.values()).sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
    }
                ...
                try {
                    ...
                    if (data.id === task.id) {
                        ...
                        await this.writeProjectConfig(projectPath, [
                            ...Array.from((await this.listProjectTasks(projectPath)).filter((t) => t.id !== task.id)),
                            validated,
                        ]);
                        return;
                    }
    ...
    async deleteTask(projectPath, taskId) {
        ...
        if (deletedTask) {
            const remaining = (await this.listProjectTasks(projectPath)).filter((t) => t.id !== taskId);
    ...
    async scanForProjects(rootPaths) {
        ...
        const found = [];
        const seen = new Set();
        const checkProject = async (projectPath, name) => {
...
//# sourceMappingURL=yaml-store.js.map